...
- `data/repository/` provides the track-library boundary and current fake implementation.
...
1. Replace `FakeTrackRepository` with Android Storage Access Framework import.
2. Add a low-latency native dual-deck audio repository behind the current ViewModel events.
3. Evaluate whether the image-backed platters should graduate to optimized GLB/USD assets inside the existing volumetric window.
